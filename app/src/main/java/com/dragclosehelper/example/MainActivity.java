...
//        RxBus.get().register(this);
        ...
        setExitSharedElementCallback(new SharedElementCallback() {
            ...
            public void onMapSharedElements(List<String> names, Map<String, View> sharedElements) {
                ...
                //更新共享元素键值对
                View view = adapter.getViewByPosition(updateIndex, R.id.rv_item_fake_iv);
                if (view != null) {
...
//        View view = adapter.getViewByPosition(integer, R.id.rv_item_fake_iv);
//        if (view != null) {
//            //以下代码已经没必要设置，因为demo中的动画效果已经全部设置在了rv_item_fake_iv上
//            view.setAlpha(1f);
//        }
//    }
    ...
    @Subscribe
    public void updateIndex(UpdateEvent updateEvent) {
        switch (updateEvent.tag){
            case "updateIndex":
                //此处使用rxbus通知对应的共享元素键值对更新
                updateIndex = updateEvent.pos;
                break;
            case "updateView":
                updateIndex = updateEvent.pos;
                //此处使用rxbus通知对应的view重新显示出来，解决在滑动返回手指拖动的过程中，看到上一个页面点击的图片显示空白的问题
                View view = adapter.getViewByPosition(updateEvent.pos, R.id.rv_item_fake_iv);
                if (view != null) {
                    //以下代码已经没必要设置，因为demo中的动画效果已经全部设置在了rv_item_fake_iv上
                    view.setAlpha(1f);
                }
