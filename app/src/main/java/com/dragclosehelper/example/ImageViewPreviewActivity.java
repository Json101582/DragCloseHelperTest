...
//                RxBus.get().post("updateIndex", position);
                BusProvider.getInstance().post(new UpdateEvent("updateView",position));
            }
        ...
        dragCloseHelper.setDragCloseListener(new DragCloseHelper.DragCloseListener() {
            ...
            public boolean intercept() {
                //默认false 不拦截 如果图片是放大状态，或者处于滑动返回状态，需要拦截
                 scale = ((PhotoView) list.get(viewPager.getCurrentItem())).getScale();
...
//                RxBus.get().post("updateView", index);
                BusProvider.getInstance().post(new UpdateEvent("updateView",index));
            ...
            public void dragClose(boolean isShareElementMode) {
                Log.e("test___", "dragCancel:拖拽状态"+isShareElementMode);
